...
6. **This is Windows.** NEVER use `/dev/stdin`, `/dev/null`, `/tmp/`, or any Unix-only paths. For temp files, use `.ai-workspace/`. For piping JSON to node, write to a file first then read it -- do NOT use `/dev/stdin`.
