...
function extractContextArtifacts({ repoRoot, activeRepoPath, createdBy = 'mind/context-artifacts', manifest = null, incremental = false }) {
  ...
  for (const a of cfg.artifacts) {
    ...
    const groupId = `artifact_${normalizeId(a.name)}`;
    ...
    nodes.push({
      ...
      kind: 'artifact',
      source: { type: 'artifact', ref: a.path, root: root },
      sourceLocation: { file: a.path },
      createdBy,
      createdAt: new Date().toISOString(),
      tags: ['artifact', `artifact:${a.name}`],
    ...
    for (const full of files) {
      ...
      const key = `artifact:${a.name}:${rel}`;
      if (incremental && manifest) {
        const prev = manifest.get(key);
        if (prev && prev.sha256 === sha) { skippedUnchanged++; continue; }
      }
      const fileId = makeIdFromLabel(`artifact_${a.name}_${rel}`, 'artifact');
      const text = buf.toString('utf8');
      nodes.push({
        id: fileId,
        label: sanitizeLabel(path.basename(rel)),
        kind: 'artifact',
        source: { type: 'artifact-file', ref: rel, file: full, artifact: a.name },
        sourceLocation: { file: rel },
        createdBy,
        createdAt: new Date().toISOString(),
        tags: ['artifact-file', `artifact:${a.name}`, path.extname(rel).slice(1) || 'plain'],
