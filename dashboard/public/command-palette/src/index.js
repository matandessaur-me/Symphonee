...
// ── Ask Symphonee ────────────────────────────────────────────────────────────
// The palette IS the ask surface. A question opens this panel (pill identity:
// accent border + glow), streams the deep answer (status milestones -> tokens),
// renders answer-derived action chips (open the note/file/task it talked
// about), and takes replies that carry the conversation. Escapes: paste a
// ready prompt into the ACTIVE terminal, or force-dispatch to the agent.
const _askConvo = [];   // {q,a} turns for the open panel (cleared on close)
...
function _askStatus(label) {
  return '<span style="font-size:12px;color:var(--subtext0);display:inline-flex;align-items:center;gap:7px;">' +
    '<span style="width:6px;height:6px;border-radius:50%;background:var(--accent);box-shadow:0 0 8px var(--accent);"></span>' +
    esc(label) + '</span>';
}

// Paste a ready-made prompt into the user's working terminal (no auto-submit).
function _askHandToTerminal(question, answerSoFar) {
  try {
    const tid = state.activeTermId || 'main';
    let prompt = 'Go deeper on this: ' + String(question).replace(/\s+/g, ' ').trim();
    const known = String(answerSoFar || '').replace(/\s+/g, ' ').trim();
    if (known) prompt += ' | What Symphonee already found in my local memory: ' + known.slice(0, 280);
    if (state.ws && state.ws.readyState === 1) {
      state.ws.send(JSON.stringify({ type: 'input', termId: tid, data: prompt }));
      toast('Prompt pasted into your terminal - press Enter to send.', 'success');
      return true;
    }
  } catch (_) { /* no live terminal */ }
  return false;
}
...
function _askRunAction(a) {
  ...
  if (a.type === 'open-note') {
    ...
    try { if (typeof switchTab === 'function') switchTab('notes'); } catch (_) {}
    try { if (typeof window.openNote === 'function') window.openNote(a.name); } catch (_) {}
  } else if (a.type === 'open-file') {
    document.getElementById('localAnswerOverlay')?.remove();
    try { if (typeof switchTab === 'function') switchTab('files'); } catch (_) {}
  ...
  try {
    ...
    for (;;) {
      ...
      while ((i = buf.indexOf('\n\n')) >= 0) {
        ...
        else if (ev.type === 'token') { text += ev.text; answerEl.innerHTML = _askMd(text); body.scrollTop = body.scrollHeight; }
        else if (ev.type === 'done' || ev.type === 'escalate') finale = ev;
      }
    }
  } catch (_) { finale = { type: 'escalate', reason: 'offline' }; }

  if (!document.getElementById('localAnswerBody')) return;   // panel closed mid-stream
  if (finale && finale.type === 'done' && finale.answer) {
    answerEl.innerHTML = _askMd(finale.answer);
    _askConvo.push({ q: question, a: finale.answer });
    // answer-derived follow-ups: open what the answer talked about
...
async function answerLocally(question, opts) {
  ...
  overlay.innerHTML =
    '<div style="background:color-mix(in srgb,var(--base) 96%,transparent);border:1px solid color-mix(in srgb,var(--accent) 28%,var(--surface2));border-radius:16px;width:680px;max-width:92vw;max-height:80vh;display:flex;flex-direction:column;box-shadow:0 0 26px -14px var(--accent),0 16px 50px rgba(0,0,0,0.55);backdrop-filter:blur(14px);">' +
      '<div style="display:flex;align-items:center;gap:9px;padding:13px 18px;border-bottom:1px solid var(--surface1);">' +
        '<span style="width:8px;height:8px;border-radius:50%;background:var(--accent);box-shadow:0 0 10px var(--accent);"></span>' +
        '<strong style="font-size:13px;">Ask Symphonee</strong>' +
        '<span id="localAnswerStatus" style="margin-left:6px;"></span>' +
        '<div style="flex:1;"></div>' +
        '<button onclick="document.getElementById(\'localAnswerOverlay\').remove()" style="background:transparent;border:none;color:var(--subtext0);cursor:pointer;font-size:16px;line-height:1;padding:4px 8px;border-radius:8px;">&times;</button>' +
      '</div>' +
      '<div id="localAnswerBody" style="padding:14px 18px 6px;overflow:auto;font-size:13px;line-height:1.6;color:var(--text);"></div>' +
      '<div id="localAnswerReplyRow" style="display:none;align-items:center;gap:8px;padding:12px 18px 4px;">' +
        '<input id="localAnswerReply" type="text" placeholder="Reply..." style="flex:1;background:var(--surface1);border:1px solid color-mix(in srgb,var(--accent) 18%,var(--surface2));border-radius:11px;color:var(--text);font-size:12.5px;padding:9px 13px;outline:none;font-family:inherit;min-width:0;"/>' +
        '<button id="localAnswerReplyGo" style="background:var(--accent);border:none;color:#11111b;font-weight:600;font-size:12px;padding:9px 14px;border-radius:8px;cursor:pointer;">Reply</button>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:10px;padding:11px 18px 14px;">' +
        '<button class="hotkey-mini" onclick="_askGoDeeper()">Go deeper in terminal</button>' +
        '<div style="flex:1;"></div>' +
  ...
  const replyGo = () => {
    ...
    if (q) { inp.value = ''; _askStream(q, opts); }
  };
  document.getElementById('localAnswerReplyGo').onclick = replyGo;
  document.getElementById('localAnswerReply').onkeydown = e => { if (e.key === 'Enter') replyGo(); };
  _askStream(question, opts);
}

function _askGoDeeper() {
  const p = state._localAnswerPending;
  const last = _askConvo[_askConvo.length - 1];
  document.getElementById('localAnswerOverlay')?.remove();
  const q = (last && last.q) || (p && p.question) || '';
  if (!_askHandToTerminal(q, last && last.a)) {
    toast('No live terminal - sending to agent instead', 'info');
    if (q) askAIFromPalette(q, { forceDispatch: true });
  }
}
