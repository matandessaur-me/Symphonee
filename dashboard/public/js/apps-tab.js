...
(() => {
  // dashboard/public/apps-tab/src/index.js
  var _appsState = {
    sessionId: null,
    running: false,
    hwnd: null,
    ...
    recipes: []
    // cached list for the current app
  };
    ...
    if (p && (p.local || p.key === "gemma") && typeof symphEnsureLocalModel === "function") {
      ...
      if (!ok) {
        ...
        if (fallback) {
          ...
          _appsState.providerKey = fallback.key;
          _appsSaveProvider(fallback.key);
        }
      }
    }
  }
  ...
  var _appsLauncher = {
    ...
    iconPending: {}
    // key → Promise, avoids double-fetch
  };
  ...
  async function appsRefreshInstalled({
    ...
    if (!force && _appsLauncher.installed.length && Date.now() - _appsLauncher.installedLoadedAt < 6e4) return;
    try {
  ...
  async function appsSaveInstructions() {
    ...
    try {
      ...
      if (statusEl) statusEl.textContent = "Saved \xB7 " + (data.bytes || 0) + " bytes";
...
This removes every learning, DO, DON'T, and note the agent has saved for this app. Useful when old sessions poisoned the file with duplicates. User-written Instructions are also cleared.`, {
    ...
    try {
      const r = await fetch("/api/apps/session/start", {
        ...
        body: JSON.stringify({
          ...
          hwnd: resolved.hwnd,
          app,
          provider: _appsState.providerKey || void 0
        })
      });
      let data = null;
      try {
  ...
  function _appsInsertUiaStep(selector, name, controlType) {
    ...
    const notes = labelBits.length ? labelBits.join(" ") : void 0;
    const step = {
      verb: "CLICK",
      target: pretty,
      text: "",
  ...
  async function _appsAutomationsLoadHistory() {
    ...
    try {
      ...
      listEl.innerHTML = runs.map((run) => {
        ...
        const dur = run.durationMs ? run.durationMs < 1e3 ? run.durationMs + "ms" : (run.durationMs / 1e3).toFixed(1) + "s" : "-";
  ...
  var _APPS_VERBS = ["CLICK", "DOUBLE_CLICK", "RIGHT_CLICK", "MIDDLE_CLICK", "DRAG", "SCROLL", "TYPE", "PRESS", "WAIT", "WAIT_UNTIL", "FIND", "VERIFY", "EXTRACT", "IF", "ELSE", "ENDIF", "REPEAT", "ENDREPEAT"];
  var _APPS_VERBS_WITH_TEXT = /* @__PURE__ */ new Set(["TYPE", "WAIT_UNTIL", "DRAG", "EXTRACT"]);
  var _APPS_VERBS_BARE = /* @__PURE__ */ new Set(["ELSE", "ENDIF", "ENDREPEAT"]);
  ...
  var _APPS_UNDO_MAX = 40;
  var _appsBuilderUndoStack = [];
  ...
  function _appsBuilderUpdate(i, field, value) {
    ...
    _appsBuilderSnapshot();
    if (value === "" || value == null) delete state._appsBuilderSteps[i][field];
    else state._appsBuilderSteps[i][field] = value;
    _appsSyncBuilderToText();
  }
  ...
  function _appsRenderBuilderRows() {
    ...
    const rows = state._appsBuilderSteps.map((s, i) => {
      ...
      <button type="button" onclick="_appsBuilderRemove(${i})" title="Delete step" style="background:transparent;border:none;color:var(--subtext0);cursor:pointer;padding:2px 6px;font-size:14px;line-height:1;">\xD7</button>
    </div>`;
    }).join("");
  ...
  function _appsTextToSteps(text) {
    ...
    for (const raw of String(text || "").split(/\r?\n/)) {
      ...
      if (noteIdx >= 0) {
        notes = rest.slice(noteIdx + 2).trim() || void 0;
        rest = rest.slice(0, noteIdx).trim();
      }
      let target, text2;
      const arrow = rest.indexOf("->");
      if (arrow >= 0) {
        target = rest.slice(0, arrow).trim() || void 0;
        text2 = rest.slice(arrow + 2).trim() || void 0;
      } else {
        target = rest || void 0;
      }
      out.push({
  ...
  async function _appsStartWithGoal(goal) {
    ...
    const recipeId = _appsState.selectedRecipeId || void 0;
    ...
    try {
      const r = await fetch("/api/apps/session/start", {
        ...
        body: JSON.stringify({
          goal: recipeId ? void 0 : goal,
          recipeId,
          inputs: runInputs || void 0,
          stepThrough: recipeId ? !!_appsState.stepThrough : void 0,
          hwnd: _appsState.hwnd,
          app: appKey,
          provider: _appsState.providerKey || void 0
        })
  ...
  function _appsUpdateRunningChrome(running) {
    ...
    if (modelBadge) {
      if (running && (_appsState.model || _appsState.provider)) {
        ...
        modelBadge.textContent = [_appsState.provider, _appsState.model].filter(Boolean).join(" \xB7 ");
      } else {
...
First paragraph..." style="width:100%;height:160px;padding:10px;border-radius:6px;border:1px solid var(--surface2);background:var(--surface1);color:var(--text);font:12px var(--font-mono);margin-bottom:8px;resize:vertical;"></textarea><div style="font-size:11px;color:var(--subtext0);margin-bottom:14px;display:none;" id="appsComExcelHint">For Excel: paste a 2D JSON array (rows of cells). Use <code style="background:var(--surface2);padding:1px 4px;border-radius:3px;">"=SUM(B2:B3)"</code> for formulas. Numbers as numbers.</div><div style="display:flex;justify-content:flex-end;gap:8px;"><button class="sy-btn sy-btn-ghost" type="button" onclick="document.getElementById('appsComModal').remove()">Cancel</button><button class="sy-btn sy-btn-primary" type="button" onclick="_appsComSubmit()" id="appsComSubmitBtn">Generate</button></div></div>`;
  ...
  var _appsSessions = /* @__PURE__ */ new Map();
  ...
  function _appsCloseSession(sessionId) {
    ...
    if (state._appsActiveSessionId === sessionId) {
      ...
      if (state._appsActiveSessionId) _appsSwitchActiveSession(state._appsActiveSessionId);
      else {
        _appsState.sessionId = null;
        const list = document.getElementById("appsLogList");
        if (list) list.innerHTML = "";
