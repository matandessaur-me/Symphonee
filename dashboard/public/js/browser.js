...
(() => {
  // dashboard/public/browser/src/index.js
  ...
  function _buildInappInspectScript(enabled) {
    ...
    function setSelected(el) {
      ...
      label.textContent = text ? (tag + ' \xB7 ' + text) : tag;
      positionSelected();
    }
  ...
  function _setBrowserAgentRunning(running) {
    ...
    const chip = document.getElementById("inappAgentChip");
    const state2 = document.getElementById("inappAgentState");
    ...
    if (state2) {
      state2.textContent = running ? "running" : "idle";
      state2.className = "inapp-agent-state" + (running ? " running" : "");
    }
  ...
  function _markAutomationActive() {
    ...
    _automationActivity.idleTimer = setTimeout(() => {
      if (Date.now() - _automationActivity.lastPing >= 8e3) {
  ...
  function handleStagehandScreencast(msg) {
    ...
    _stagehandCast.idleTimer = setTimeout(() => {
      if (Date.now() - _stagehandCast.lastFrame >= 8e3) {
  ...
  function handleBrowserAgentStep(msg) {
    ...
    switch (msg.kind) {
      case "provider": {
        const state2 = document.getElementById("inappAgentState");
        if (state2) {
          state2.textContent = msg.label || msg.provider || "running";
        }
        break;
      }
      case "user":
  ...
  async function _runPageMap() {
    ...
    if (url && _pageMapCache.url === url && _pageMapCache.map && Date.now() - _pageMapCache.ts < 5 * 60 * 1e3) {
  ...
  async function _sendBrowserAgentTask(task, displayText, options) {
    ...
    try {
      ...
      const cached = curUrl && _pageMapCache.url === curUrl && _pageMapCache.map && Date.now() - _pageMapCache.ts < 5 * 60 * 1e3;
    ...
    try {
      const res = await fetch("/api/browser/agent/chat", {
        ...
        body: JSON.stringify({
          task: composedTask,
          threadId: _browserAgentState.threadId,
          provider: _browserAgentState.provider || void 0
        })
      });
      ...
      if (!res.ok) {
        ...
        const state2 = document.getElementById("inappAgentState");
        if (state2) state2.title = data.label + " (" + (data.model || "") + ")";
      }
    } catch (e) {
  ...
  var _BRAND_EXTRACT_SCRIPT = `(function(){
  function parseColor(str){
    if (!str) return null;
    var m = String(str).match(/rgba?\\((-?[0-9.]+)[,\\s]+(-?[0-9.]+)[,\\s]+(-?[0-9.]+)(?:[,/\\s]+([0-9.]+%?))?\\)/);
    if (!m) return null;
  ...
  async function _saveBrandToNote() {
    ...
    const md = [`# ${brandName}`, "", data.description ? `> ${data.description}` : null, "", `- **URL:** ${data.url}`, data.themeColor ? `- **Theme color:** \`${data.themeColor}\`` : null, data.ogImage ? `- **Logo:** ${data.ogImage}` : null, data.favicon ? `- **Favicon:** ${data.favicon}` : null, "", "## Palette", palette || "_None detected._", "", "## Typography", fonts || "_None detected._", "", `_Captured ${(/* @__PURE__ */ new Date()).toISOString()}_`].filter((l) => l !== null).join("\n");
        ...
        <div class="box-model-margin">
          ...
          <div class="box-model-border">
            ...
            <div class="box-model-padding">
              ...
              <div class="box-model-content">${bm.width} \xD7 ${bm.height}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    const blocks = Object.entries(groups).map(([groupName, keys]) => {
      const rows = keys.filter((k) => styles[k] && styles[k].trim()).map((k) => {
        ...
        if (isColor(k, v)) {
          ...
          const K2 = JSON.stringify(k).replace(/"/g, "&quot;");
          const H = JSON.stringify(hex).replace(/"/g, "&quot;");
          swatch = `<span class="chip" style="background:${_escapeHtml(v)}" onclick="_openColorEditorAtChip(this, ${S}, ${K2}, ${H})"></span>`;
        }
  ...
  var _PATCH_STORAGE_KEY = "symphonee.browser.patches";
  ...
  var _READER_FONT_SIZES = ["15px", "17px", "19px", "21px", "24px"];
...
${snippet}${a.text.length > 6e3 ? "\n\n[truncated]" : ""}`;
  ...
  var _SITE_AUDIT_SCRIPT = `(function(){
  ...
  async function _runInappEmulatePanel() {
    ...
    const devOpts = _EMULATE_DEVICES.map((d) => `<option value="${d.id}" ${_emulateState.device === d.id ? "selected" : ""}>${_escapeHtml(d.label)}${d.w ? " \u2014 " + d.w + "\xD7" + d.h + " @" + d.dpr + "x" : ""}</option>`).join("");
    _setInappToolsBodyHtml(`
    ...
    <div class="code-inspect-group"><div class="code-inspect-group-title">Throttling</div>
      <div class="quick-edit-grid" style="grid-template-columns: 110px 1fr;">
        ...
        <select id="emCpu" onchange="_applyEmulateThrottle()">
          ${[1, 2, 4, 6, 10, 20].map((r) => `<option value="${r}" ${_emulateState.cpuRate === r ? "selected" : ""}>${r === 1 ? "No throttling" : r + "\xD7 slower"}</option>`).join("")}
        </select>
      </div>
    </div>
    <div class="inapp-tools-actions" style="margin:8px -12px -12px;">
  ...
  async function _applyEmulateDevice() {
    ...
    try {
      if (id === "off" || !d.w) {
        ...
        toast(d.label + " \u2014 " + d.w + "\xD7" + d.h, "success", {
          duration: 1400
        });
      }
    } catch (e) {
  ...
  function _issueSummary(it) {
    ...
    return {
      code,
      line: parts.join(" \xB7 ").slice(0, 180)
    };
  }
    ...
    for (const [code, list] of byCode.entries()) {
      ...
      <div class="sym-patch-card">
        <div class="sym-patch-head">
          ...
          <span class="sym-patch-when">${list.length}\xD7</span>
        </div>
        ${items}
      </div>
    `);
  ...
  function _renderInappAuditPanel(d) {
    ...
    const perfChecks = d.timing ? [_auditCheck(d.timing.ttfb < 600, d.timing.ttfb < 1500, `<strong>TTFB:</strong> ${_fmtMs(d.timing.ttfb)} <span style="color:var(--subtext0);">(target &lt;600 ms)</span>`), _auditCheck(d.timing.domContentLoaded < 2500, d.timing.domContentLoaded < 5e3, `<strong>DOM ready:</strong> ${_fmtMs(d.timing.domContentLoaded)}`), _auditCheck(d.timing.loadEvent < 4e3, d.timing.loadEvent < 8e3, `<strong>Load event:</strong> ${_fmtMs(d.timing.loadEvent)}`), _auditCheck(d.resources.totalTransferBytes < 2 * 1024 * 1024, d.resources.totalTransferBytes < 5 * 1024 * 1024, `<strong>Transfer size:</strong> ${_fmtBytes(d.resources.totalTransferBytes)} across ${d.resources.total} resources`), _auditCheck(d.nodeCount < 1500, d.nodeCount < 3e3, `<strong>DOM size:</strong> ${d.nodeCount.toLocaleString()} elements`)].join("") : '<div class="inapp-tools-empty" style="padding:10px;">No navigation timing available (try reloading the page).</div>';
  ...
  async function _saveAuditToNote() {
    ...
    lines.push(`**Captured:** ${(/* @__PURE__ */ new Date()).toISOString()}`);
    lines.push("");
    lines.push("## SEO");
