...
(() => {
  // dashboard/public/notifications/src/index.js
  function openActivityStats() {
    ...
    for (const t of recent) {
      ...
      const dayIdx = Math.floor((now - d.getTime()) / (24 * 60 * 60 * 1e3));
  ...
  var NOTIF_SOUND_KEY = "symphonee-notif-sound-v1";
  ...
  function playNotifSound(kind) {
    ...
    try {
      ...
      const tones = {
        success: [[880, 0, 0.08], [1320, 0.1, 0.08]],
        error: [[220, 0, 0.14], [160, 0.16, 0.16]],
        warning: [[660, 0, 0.1], [660, 0.14, 0.1]],
        info: [[520, 0, 0.09]]
      };
  ...
  var NOTIF_KEY = "symphonee-notifications";
  var NOTIF_MAX = 50;
  state._notifs = [];
  var _paletteNotifyTasks = /* @__PURE__ */ new Set();
  var _paletteDispatchToasts = /* @__PURE__ */ new Map();
  ...
  function renderNotifList() {
    ...
    el.innerHTML = state._notifs.map((n) => {
      ...
      const actions = hasTask ? '<div class="notif-item-actions"><button class="notif-item-btn" data-act="open" data-task="' + esc2(n.taskId) + '"><i data-lucide="external-link" style="width:11px;height:11px;"></i>Open</button><button class="notif-item-btn primary" data-act="reply" data-task="' + esc2(n.taskId) + '"><i data-lucide="corner-down-left" style="width:11px;height:11px;"></i>Reply</button></div>' : "";
      return '<div class="notif-item ' + (n.read ? "" : "unread") + '" data-id="' + n.id + '"><div class="notif-item-head"><span class="notif-item-icon"><i data-lucide="' + esc2(n.icon || "bell") + '"></i></span><span class="notif-item-title" title="' + esc2(n.title) + '">' + esc2(n.title) + '</span><span class="notif-item-time">' + _relTime(n.createdAt) + '</span><button class="notif-item-del" data-act="delete" title="Dismiss" aria-label="Dismiss notification"><i data-lucide="x"></i></button></div><div class="notif-item-body">' + esc2(n.body) + "</div>" + actions + "</div>";
    }).join("");
