...
(() => {
  // dashboard/public/orchestrator/src/index.js
  state.orchTasks = [];
  state.orchAgents = [];
  ...
  function syncOrchScopeFilter() {
    ...
    if (!el) return;
    if (!state.activeSpace && el.value === "space") el.value = "all";
    else if (state.activeSpace && !el.dataset.userTouched) el.value = "space";
  }
  ...
  var _orchTaskOutput = /* @__PURE__ */ new Map();
  function renderOrchTasks() {
    ...
    function renderTaskCard(t, depth) {
      ...
      const accOpen = function(label, defaultOpen) {
        return (saved[label] !== void 0 ? saved[label] : defaultOpen) ? " open" : "";
      };
    ...
    if (hasRunning && !state._orchTimerInterval) {
      state._orchTimerInterval = setInterval(updateOrchTimers, 1e3);
...
${rawPrompt}`;
  ...
  function handleOrchestratorEvent(msg) {
    ...
    if (msg.event === "task-output" && msg.taskId) {
      ...
      if (outputEl) {
        const full = _orchTaskOutput.get(msg.taskId);
        outputEl.textContent = full.substring(full.length - 2e3);
