...
(() => {
  // dashboard/public/themes/src/index.js
  var THEME_STORAGE_KEY = "symphonee-themes";
  var ACTIVE_THEME_KEY = "symphonee-active-theme";
  state._themeEditorDirty = false;
  state._editorMode = "dark";
  var BUILTIN_THEMES = [{
    id: "industrial-blue",
  ...
  var ALL_CSS_KEYS = ["--crust", "--mantle", "--base", "--surface0", "--surface1", "--surface2", "--overlay0", "--overlay1", "--subtext0", "--subtext1", "--text", "--blue", "--sapphire", "--green", "--yellow", "--peach", "--red", "--mauve", "--teal", "--accent"];
  ...
  function _initEditorPickers() {
    ...
    if (stored.startsWith("__builtin_")) {
      ...
      const t = getSavedThemes().find((t2) => t2.name === stored);
      if (t && t.mode) {
        mode = t.mode;
        tint = t.tint;
        text = t.text;
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        for (const t of imported) {
          if (!existing.some((e2) => e2.name === t.name)) {
            existing.push(t);
            added++;
          }
        }
