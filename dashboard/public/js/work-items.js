...
(() => {
  // dashboard/public/work-items/src/index.js
  ...
  function notesFetch(url, init) {
    ...
    if (init.body && typeof init.body === "string") {
      try {
        ...
        if (obj && typeof obj === "object" && obj.ns === void 0) {
          obj.ns = ns;
          init = {
            ...init,
  ...
  var _expandedParents = new Set(JSON.parse(localStorage.getItem("symphonee-expanded-parents") || "[]"));
  ...
  function renderBoard() {
    ...
    for (const wi of filtered) {
      const state2 = wi.state;
      if (buckets[state2]) buckets[state2].push(wi);
      else if (state2 === "Done") buckets.Closed.push(wi);
      else buckets.New.push(wi);
    }
    ...
    for (const [state2, items] of Object.entries(buckets)) {
      const container = document.getElementById(`board${state2}`);
      const count = document.getElementById(`boardCount${state2}`);
      if (!container) continue;
      if (state2 === "Closed" && window.state.totalClosedCount > items.length) {
      ...
      const sameColChildIds = /* @__PURE__ */ new Set();
      ...
      let html = "";
      for (const wi of items) {
        if (sameColChildIds.has(wi.id)) continue;
      ...
      if (state2 === "Closed" && window.state.hasMoreClosed) {
  ...
  var _collapsedBacklogParents = new Set(JSON.parse(localStorage.getItem("symphonee-collapsed-backlog") || "[]"));
  ...
  function renderBacklog() {
    ...
    let html = "";
    for (const wi of state.workItems) {
      if (childIds.has(wi.id)) continue;
  ...
  function acceptMention(name) {
    ...
    const newPos = state._mentionStart + name.length + 2;
  ...
  async function startWorking(wiId) {
    ...
    try {
      ...
      const sendCtx = () => setTimeout(() => sendCommand(ctx), 2e3);
      if (!aiLaunched) {
        setTimeout(() => {
          launchAi();
          sendCtx();
  ...
  function updateSprintCard(iteration) {
    ...
    const total = (finish - start) / 864e5;
    const elapsed = (now - start) / 864e5;
    const remaining = Math.max(0, Math.ceil((finish - now) / 864e5));
  ...
  async function loadAreas() {
    try {
      ...
      for (const area of areas) {
        ...
        const indent = parts.length > 1 ? "\xA0\xA0".repeat(parts.length - 1) : "";
  ...
  function updateActivityFeed() {
    ...
    function activityLabel(wi) {
      ...
      const hoursSince = (Date.now() - changed) / 36e5;
      if (state2 === "Closed" || state2 === "Done") return {
        text: "Closed",
        color: "var(--subtext0)",
        icon: "check-circle"
      };
      if (state2 === "Resolved") return {
        text: "Resolved",
        color: "var(--mauve)",
        icon: "check"
      };
      ...
      if (state2 === "New" && hoursSince < 48) {
        const isRealCreation = created && Math.abs(changed - created) < 12e4;
    ...
    <div class="activity-item" onclick="viewWorkItem(${wi.id})" style="cursor:pointer;">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="activity-time">
          ${wi.assignedTo ? `${esc(wi.assignedTo.split(" ")[0])} \xB7 ` : ""}${formatDate(wi.changedDate)}
        </div>
      </div>
    </div>`;
    }).join("");
