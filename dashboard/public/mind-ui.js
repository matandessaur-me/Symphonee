...
(() => {
  ...
  function renderWakeupStoryLine(line) {
    ...
    if (/:$/.test(clean) && !clean.startsWith("- ") && !clean.startsWith("[") && !clean.startsWith("->") && !clean.startsWith("~>") && !clean.startsWith("?>")) {
      return `<div style="font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:var(--subtext0);margin-top:4px;">${escapeHtml2(clean.slice(0, -1))}</div>`;
    }
    ...
    return `<div style="padding:8px 10px;background:${bg};border:1px solid var(--surface0);border-radius:6px;font-size:11px;color:var(--text);line-height:1.5;overflow-wrap:anywhere;">${escapeHtml2(text)}</div>`;
  }
  function renderWakeupPreview(data) {
    ...
    return `
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;">
        <div style="padding:12px;background:var(--base);border:1px solid var(--surface0);border-radius:6px;">
          ...
          <div style="display:grid;grid-template-columns:72px minmax(0,1fr);gap:8px;font-size:11px;line-height:1.5;">
            ${identityRows.map(([label, value]) => `<div style="color:var(--subtext0);">${escapeHtml2(label)}</div><div style="color:var(--text);overflow-wrap:anywhere;">${escapeHtml2(value)}</div>`).join("")}
          </div>
        </div>
        <div style="padding:12px;background:var(--base);border:1px solid var(--surface0);border-radius:6px;">
          ...
          <div style="font-size:11px;color:var(--text);line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere;">${escapeHtml2(parsed.preamble || "No repo preamble found.")}</div>
        </div>
      </div>
      <div style="padding:12px;background:var(--base);border:1px solid var(--surface0);border-radius:6px;">
        <div style="font-size:10px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:var(--subtext0);margin-bottom:8px;">${escapeHtml2(parsed.l1Title.replace(/^L1 -\s*/, ""))}</div>
      ...
      <details style="background:var(--base);border:1px solid var(--surface0);border-radius:6px;padding:10px 12px;">
        ...
        <pre style="margin:10px 0 0;background:var(--mantle);padding:12px;border-radius:6px;font-size:11px;line-height:1.6;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;color:var(--text);border:1px solid var(--surface0);">${escapeHtml2(parsed.raw || "(empty)")}</pre>
      </details>`;
  }
  ...
  function _renderAnchorList() {
    ...
    host.innerHTML = shown.map((a) => {
      ...
      return '<div class="specs-item" data-id="' + escapeHtml2(a.id) + '" style="display:flex;align-items:flex-start;gap:8px;padding:8px 9px;border-radius:8px;cursor:pointer;margin-bottom:2px;' + (sel ? "background:color-mix(in srgb, var(--accent) 20%, var(--surface0));" : "") + '"><span style="width:8px;height:8px;border-radius:50%;background:' + color + ";flex-shrink:0;box-shadow:0 0 6px " + color + '66;margin-top:3px;"></span><span style="flex:1;min-width:0;display:flex;flex-direction:column;gap:2px;"><span style="display:flex;align-items:center;gap:6px;"><span style="flex:1;font-size:12.5px;font-weight:500;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml2(a.label) + '</span><span style="font-size:8.5px;color:var(--subtext0);text-transform:uppercase;letter-spacing:.4px;flex-shrink:0;padding:1px 5px;border-radius:6px;background:' + color + '22;">' + escapeHtml2(a.kind) + "</span></span>" + (breakdown ? '<span style="font-size:10px;color:var(--subtext0);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml2(breakdown) + "</span>" : "") + "</span></div>";
  ...
  async function _specsSelect(anchor, keepGraph) {
    ...
    if (header) header.innerHTML = '<span style="font-size:13px;font-weight:600;color:var(--text);">' + escapeHtml2(anchor.label) + '</span><span style="font-size:9px;color:var(--subtext0);text-transform:uppercase;letter-spacing:.3px;">' + escapeHtml2(anchor.kind) + '</span><span id="specsCounts" style="font-size:11px;color:var(--subtext0);">building...</span><span style="flex:1;"></span><button id="specsExportBtn" class="tab-bar-btn" style="font-size:11px;">Export KIT</button>';
  ...
  function _renderSpecGraph(kit) {
    ...
    try {
      const fg = window.ForceGraph()(host).graphData({ nodes, links }).backgroundColor("rgba(0,0,0,0)").nodeRelSize(4).nodeColor((n) => n.color).nodeLabel((n) => escapeHtml2(n.label) + "  [" + n.kind + "]").linkColor(() => "rgba(150,150,180,0.22)").linkWidth(0.6).onNodeClick((n) => {
        try {
          showNodeDetail(n.id);
        } catch (_) {
        }
  ...
  var MindSkills = {
    ...
    md(s) {
      try {
        return typeof window.renderMarkdownToHtml === "function" ? window.renderMarkdownToHtml(s || "") : escapeHtml2(s || "").replace(/\n/g, "<br>");
      } catch (_) {
    ...
    renderList() {
      ...
      if (props.length) {
        ...
        for (const p of props) {
          html += '<div style="background:color-mix(in srgb, var(--yellow) 10%, var(--surface0));border:1px solid color-mix(in srgb, var(--yellow) 30%, var(--surface2));border-radius:8px;padding:9px 10px;margin-bottom:6px;"><div style="font-size:12.5px;font-weight:600;color:var(--text);margin-bottom:3px;">' + escapeHtml2(p.name) + '</div><div style="font-size:10.5px;color:var(--subtext0);line-height:1.4;margin-bottom:7px;">' + escapeHtml2(p.description) + `</div><div style="display:flex;gap:6px;"><button class="tab-bar-btn" style="font-size:11px;" onclick="MindSkills.review('` + escapeHtml2(p.id) + `')">Review</button><button style="font-size:11px;padding:4px 9px;background:var(--green);border:none;border-radius:5px;color:#000;font-weight:600;cursor:pointer;" onclick="MindSkills.accept('` + escapeHtml2(p.id) + `')">Accept</button><button style="font-size:11px;padding:4px 9px;background:var(--surface1);border:1px solid var(--surface2);border-radius:5px;color:var(--red);cursor:pointer;" onclick="MindSkills.reject('` + escapeHtml2(p.id) + `')">Reject</button></div></div>`;
        }
      ...
      for (const s of this.state.skills) {
        ...
        html += `<div onclick="MindSkills.select('` + escapeHtml2(s.id) + `')" style="cursor:pointer;border-radius:8px;padding:9px 10px;margin-bottom:4px;` + (sel ? "background:color-mix(in srgb, var(--accent) 18%, var(--surface0));" : "") + '"><div style="font-size:12.5px;font-weight:600;color:var(--text);">' + escapeHtml2(s.name) + '</div><div style="font-size:10.5px;color:var(--subtext0);line-height:1.4;">' + escapeHtml2(s.description) + "</div></div>";
      }
      host.innerHTML = html;
    },
    detailEmpty() {
    ...
    async select(id) {
      ...
      d.innerHTML = '<div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px;"><div style="flex:1;min-width:0;"><div style="font-size:16px;font-weight:700;color:var(--text);">' + escapeHtml2(skill.name) + '</div><div style="font-size:12px;color:var(--subtext0);margin-top:3px;">' + escapeHtml2(skill.description) + '</div><div style="margin-top:7px;display:flex;gap:5px;flex-wrap:wrap;align-items:center;">' + (skill.tags || []).map((t) => '<span style="font-size:9.5px;background:var(--surface1);color:var(--subtext0);padding:2px 7px;border-radius:8px;">' + escapeHtml2(t) + "</span>").join("") + '<code style="font-size:9.5px;color:var(--overlay1);">' + escapeHtml2(skill.id) + `</code></div></div><button class="tab-bar-btn" style="font-size:12px;" onclick="MindSkills.edit()">Edit</button><button class="tab-bar-btn" style="font-size:12px;color:var(--red);" onclick="MindSkills.del('` + escapeHtml2(skill.id) + `')">Delete</button></div><div style="border-top:1px solid var(--surface0);padding-top:14px;font-size:13px;line-height:1.55;color:var(--text);">` + this.md(skill.body || "") + "</div>";
    },
    edit() {
    ...
    editForm(skill, opts) {
      ...
      d.innerHTML = '<div style="display:flex;flex-direction:column;gap:9px;height:100%;">' + (opts.accept ? '<div style="font-size:11px;color:var(--yellow);">Reviewing a proposed skill -- edit then Accept to add it to the corpus.</div>' : "") + '<input id="skmName" placeholder="Skill name (imperative)" value="' + escapeHtml2(skill.name || "") + '" style="' + inp + '"><input id="skmId" placeholder="id (kebab-case; blank = from name)" value="' + escapeHtml2(skill.id || "") + '" ' + (skill.id && !opts.accept ? "readonly" : "") + ' style="' + inp + (skill.id && !opts.accept ? "opacity:.6;" : "") + '"><input id="skmDesc" placeholder="One-line description: what it does + when to use it" value="' + escapeHtml2(skill.description || "") + '" style="' + inp + '"><input id="skmWhen" placeholder="when: short trigger phrase (optional)" value="' + escapeHtml2(skill.when || "") + '" style="' + inp + '"><input id="skmTags" placeholder="tags, comma, separated" value="' + escapeHtml2((skill.tags || []).join(", ")) + '" style="' + inp + '"><textarea id="skmBody" placeholder="Body markdown: ## Use when / ## Steps (primary path) / ## Safety / ## Verification" style="' + inp + 'flex:1;min-height:240px;font-family:var(--font-mono,monospace);line-height:1.5;resize:none;">' + escapeHtml2(skill.body || "") + '</textarea><div style="display:flex;gap:8px;justify-content:flex-end;"><button class="tab-bar-btn" onclick="MindSkills.cancel()">Cancel</button><button style="font-size:12px;padding:7px 16px;background:var(--accent);border:none;border-radius:6px;color:#000;font-weight:600;cursor:pointer;" onclick="MindSkills.save(' + (opts.accept ? "'" + escapeHtml2(skill.id) + "'" : "null") + ')">' + (opts.accept ? "Accept" : "Save") + "</button></div></div>";
    },
    cancel() {
      this.state.editing = false;
        ...
        <div class="mind-smart-grid">
          <section class="mind-smart-card mind-smart-card-search">
            ...
            <div class="mind-smart-form">
              <input id="mindSmartQ" type="text" placeholder="Ask anything: auth flow, orchestrator mount, tsconfig aliases..." autofocus value="${escapeHtml2(last.q || "")}">
  ...
  async function refreshSmartHealth() {
    try {
      ...
      const v = r.vectors || {};
      const eOk = e.ok ? `<span style="color:var(--green);">${escapeHtml2(e.provider || "")} ok (${e.latencyMs || 0}ms, ${e.dimensions || "?"}d)</span>` : `<span style="color:var(--subtext0);">semantic provider not configured</span>`;
      const vOk = v.count > 0 ? `<span style="color:var(--green);">${v.count} vectors indexed (${v.dim}d, ${escapeHtml2(v.provider || "")})</span>` : '<span style="color:var(--yellow);">no vectors yet</span>';
  ...
  async function runSmart() {
    ...
    try {
      ...
      out.innerHTML = ranked.map(([id, score], rank) => {
        ...
        const label = node ? escapeHtml2(node.label) : escapeHtml2(id);
        const kind = node ? `<span style="color:var(--subtext0);font-size:10px;">[${escapeHtml2(node.kind || "")}]</span>` : "";
        return `
          <a href="#" class="mind-smart-link" data-id="${escapeHtml2(id)}">
      ...
      out.innerHTML = `<span style="color:var(--red);">error: ${escapeHtml2(e.message || String(e))}</span>`;
    }
  }
  ...
  function renderImpactList(files) {
    ...
    list.innerHTML = files.slice(0, 800).map((f) => {
      ...
      return `
        <button class="mind-impact-row${_impactSelectedPath === f.path ? " active" : ""}" data-path="${escapeHtml2(f.path)}">
          <span class="mind-impact-row-name">${escapeHtml2(basenameOf(f.path))}</span>
          <span class="mind-impact-row-file">${escapeHtml2(f.path)}</span>
        ...
        <div class="mind-impact-cycle-list">
          ${cycles.map((cycle) => `
            <div class="mind-impact-cycle-path">
              ${(cycle || []).map((p) => `<span title="${escapeHtml2(p)}">${escapeHtml2(basenameOf(p))}</span>`).join("<b>to</b>")}
            </div>`).join("")}
        </div>
      </details>`;
  }
  async function selectImpactFile(path) {
    ...
    detail.innerHTML = `
      <div class="mind-impact-detail-head">
        <div>
          <div class="mind-impact-detail-name">${escapeHtml2(basenameOf(path))}</div>
          <div class="mind-impact-detail-file">${escapeHtml2(path)}</div>
        </div>
    ...
    try {
      ...
      depsBody.innerHTML = data.dependents && data.dependents.length ? data.dependents.map((d) => `
          <button class="mind-impact-link" data-path="${escapeHtml2(d.path)}">
            \u2190 <span class="mind-impact-call-name">${escapeHtml2(basenameOf(d.path))}</span>
            <span class="mind-impact-call-file">${escapeHtml2(d.path)}</span>
      ...
      impBody.innerHTML = data.imports && data.imports.length ? data.imports.map((d) => `
          <button class="mind-impact-link${d.external ? " external" : ""}" data-path="${escapeHtml2(d.path)}" ${d.external ? "disabled" : ""}>
            \u2192 <span class="mind-impact-call-name">${escapeHtml2(d.external ? d.path.replace(/^ext_/, "") : basenameOf(d.path))}</span>
            <span class="mind-impact-call-file">${escapeHtml2(d.external ? "(external)" : d.path)}</span>
      ...
      symsBody.innerHTML = data.symbols && data.symbols.length ? data.symbols.slice(0, 50).map((s) => `
          <div class="mind-impact-call">
            ...
            <span class="mind-impact-call-name">${escapeHtml2(s.name)}</span>
      ...
      const hops = Object.keys(data.filesByDepth || {}).map((hop) => {
        ...
        return `
          <div class="mind-impact-hop">
            ...
            ${files.map((f) => `<div class="mind-impact-hop-file" style="border-left-color:${c};">${escapeHtml2(f)}</div>`).join("")}
          </div>`;
      }).join("");
      body.innerHTML = `
        <div style="font-size:11px;color:var(--subtext0);margin-bottom:8px;">${data.totalFiles} file${data.totalFiles === 1 ? "" : "s"} would break if you delete or rename <b style="color:var(--text);">${escapeHtml2(basenameOf(path))}</b>${data.truncated ? " (truncated)" : ""}</div>
  ...
  async function renderKnowledge() {
    ...
    try {
      ...
      const list = $("mindKnowledgeList");
      if (list) list.innerHTML = `<div class="mind-knowledge-empty">Error loading artifacts: ${escapeHtml2(e.message || String(e))}</div>`;
    }
  }
  ...
  function renderKnowledgeRepoCard(g) {
      ...
      ${declared.map((a) => `
        <article class="mind-artifact-card">
          <header class="mind-artifact-head">
            <span class="mind-artifact-name">${escapeHtml2(a.name)}</span>
          ...
          <div class="mind-artifact-path">${escapeHtml2(a.path || "")}</div>
          <div class="mind-artifact-desc">${escapeHtml2(a.description || "")}</div>
        </article>`).join("")}` : "";
    const suggestedHtml = suggested.length ? `
      ...
      <div class="mind-knowledge-suggest" data-repo="${escapeHtml2(g.repo)}">
        ${suggested.map((s, i) => `
          <label class="mind-suggest-row">
            <input type="checkbox" data-idx="${i}" checked>
            <div class="mind-suggest-body">
              <div class="mind-suggest-head">
                <span class="mind-suggest-name">${escapeHtml2(s.name)}</span>
                <span class="mind-suggest-path">${escapeHtml2(s.path)}</span>
              </div>
              <div class="mind-suggest-desc">${escapeHtml2(s.description)}</div>
            </div>
          </label>`).join("")}
      </div>
      <button class="mindmap-ribbon-btn primary mind-knowledge-create-btn" data-action="create-artifacts" data-repo="${escapeHtml2(g.repo)}">
    ...
    return `
      <section class="mind-knowledge-repo">
        <header class="mind-knowledge-repo-head">
          <div class="mind-knowledge-repo-name">${escapeHtml2(g.repo)}</div>
          <div class="mind-knowledge-repo-path">${escapeHtml2(g.repoPath || "")}</div>
        </header>
        ${declaredHtml}
        ${suggestedHtml}
        ${emptyHtml}
    ...
    main.innerHTML = `
      <div class="mind-card" style="max-width:980px;margin:0 auto;">
        ...
        <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;align-items:center;">
          <input id="mindWakeupQ" type="text" placeholder="Optional: preview a sample task..." style="flex:1;min-width:260px;padding:8px 10px;background:var(--surface0);border:1px solid var(--surface1);border-radius:4px;color:var(--text);font-size:12px;" value="${escapeHtml2(cached.question || "")}">
  ...
  async function refreshWakeupOutput() {
    ...
    try {
      ...
      const out = $("mindWakeupOut");
      if (out) out.innerHTML = `<div style="background:var(--mantle);border:1px solid var(--surface0);border-radius:6px;padding:14px;color:#f38ba8;">Error: ${escapeHtml2(e.message || String(e))}</div>`;
    }
  }
  async function runQueryFromUi() {
    ...
    try {
      ...
      if (out) out.innerHTML = `<div style="color:#f38ba8;">Error: ${escapeHtml2(e.message || String(e))}</div>`;
    }
  }
    ...
    if (!data || data.empty || !data.nodes || !data.nodes.length) {
      ...
      out.innerHTML = `
        <div style="background:var(--mantle);border:1px dashed var(--surface1);border-radius:6px;padding:18px;color:var(--subtext0);font-size:12px;line-height:1.6;">
          <div style="color:var(--text);font-weight:600;margin-bottom:6px;">${escapeHtml2(note)}</div>
    ...
    out.innerHTML = `
      <div style="background:var(--mantle);border:1px solid var(--surface0);border-radius:6px;padding:10px 14px;font-size:11px;color:var(--subtext0);">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:14px;font-variant-numeric:tabular-nums;">
          ...
          <span>~<strong style="color:var(--text);">${data.estTokens || 0}</strong> tokens</span>
          <span>as of <strong style="color:var(--text);">${escapeHtml2(data.asOf || "now (timeless)")}</strong></span>
        </div>
      </div>
      <div style="margin-top:8px;display:grid;grid-template-columns:1fr;gap:3px;">
        ${nodes.map((n) => `
          <a href="#" class="mind-godlink" data-id="${escapeHtml2(n.id)}" style="display:grid;grid-template-columns:120px 1fr;align-items:center;gap:10px;padding:7px 12px;background:var(--mantle);border-radius:4px;text-decoration:none;color:var(--text);font-size:12px;border:1px solid transparent;">
            <span style="font-size:9px;color:var(--subtext0);text-transform:uppercase;letter-spacing:0.5px;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">[${escapeHtml2(n.kind)}]</span>
            <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;" title="${escapeHtml2(n.label || n.id)}">${escapeHtml2(n.label || n.id)}</span>
          </a>`).join("")}
      </div>`;
  ...
  async function refreshCliCoverage() {
    ...
    try {
      ...
      for (const cli of klist) {
        ...
        html += `
          <tr>
            <td style="padding:6px 8px;border-bottom:1px solid color-mix(in srgb,var(--surface0) 50%,transparent);"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${colorOf(cli)};margin-right:6px;"></span>${escapeHtml2(cli)}</td>
            <td style="padding:6px 8px;border-bottom:1px solid color-mix(in srgb,var(--surface0) 50%,transparent);font-family:monospace;color:${memFile ? "var(--text)" : "var(--subtext0)"};">${memFile ? escapeHtml2(memFile) : "\u2014"}</td>
      ...
      el.innerHTML = `<span style="color:var(--red);">error: ${escapeHtml2(e.message || String(e))}</span>`;
    }
  }
  function statCard(label, value, color, hint) {
    return `<div class="mind-stat-card"><div class="mind-stat-label">${escapeHtml2(label)}</div><div class="mind-stat-value" style="color:${color}">${escapeHtml2(String(value))}</div>${hint ? `<div class="mind-stat-hint">${escapeHtml2(hint)}</div>` : ""}</div>`;
  }
  function barChart(rows, maxRows) {
    ...
    return '<div class="mind-list">' + rows.slice(0, maxRows || 12).map((r) => {
      ...
      return `<div class="mind-bar"><div class="mind-bar-label" title="${escapeHtml2(r.label)}${r.hint ? " (" + escapeHtml2(r.hint) + ")" : ""}">${escapeHtml2(r.label)}</div><div class="mind-bar-track"><div class="mind-bar-fill" style="width:${pct.toFixed(1)}%;background:${color}"></div></div><div class="mind-bar-num">${escapeHtml2(String(r.value))}</div></div>`;
    }).join("") + "</div>";
  }
  ...
  function godRow(x, max) {
    ...
    return `<a href="#" class="mind-rank-row mind-godlink" data-id="${escapeHtml2(x.id)}"><span class="mind-rank-label" title="${escapeHtml2(x.label)}">${escapeHtml2(x.label)}</span><span class="mind-rank-track"><span class="mind-rank-fill" style="width:${pct.toFixed(1)}%;background:#fab387"></span></span><span class="mind-rank-meta"><strong>${x.degree}</strong><span>connections</span></span></a>`;
  }
  function contributorRow(label, value, max) {
    const pct = value / max * 100;
    return `<div class="mind-rank-row"><span class="mind-rank-label" title="${escapeHtml2(label)}">${escapeHtml2(label)}</span><span class="mind-rank-track"><span class="mind-rank-fill" style="width:${pct.toFixed(1)}%;background:${cliColor(label)}"></span></span><span class="mind-rank-meta"><strong>${value}</strong><span>entries</span></span></div>`;
  }
  function communityRow(cid, c, maxSize) {
    ...
    return `<a href="#" class="mind-rank-row" data-cid="${escapeHtml2(cid)}"><span class="mind-rank-label" title="${escapeHtml2(c.label)}">#${cid} ${escapeHtml2(c.label)}</span><span class="mind-rank-track"><span class="mind-rank-fill" style="width:${pct.toFixed(1)}%;background:${communityColor(parseInt(cid, 10))}"></span></span><span class="mind-rank-meta"><strong>${c.size} nodes</strong><span>${cohesionPct}% cohesion</span></span></a>`;
  }
  function convRow(n) {
    ...
    const tags = (n.tags || []).filter((t) => !["cli-session", "conversation", "drawer", "verbatim", "user", "assistant", cli].includes(t)).slice(0, 3).map((t) => `<span style="font-size:9px;color:var(--subtext0);">#${escapeHtml2(t)}</span>`).join(" ");
    ...
    return `<div class="mind-feed-row" data-id="${escapeHtml2(n.id)}">
      <div class="mind-feed-meta"><span class="mind-cli-badge" style="background:${color};color:#1e1e2e;">${escapeHtml2(cli)}</span>${typeBadge}<span>${escapeHtml2(date)}</span>${tags}</div>
      <div class="mind-feed-text">${escapeHtml2(n.preview || n.label)}</div>
    </div>`;
  }
  function surpriseRow(g, s) {
    return `<div class="mind-surprise-row">
      <a href="#" data-id="${escapeHtml2(s.source)}" class="mind-surprise-src">${escapeHtml2(nodeLabel(g, s.source))}</a>
      <span class="mind-surprise-rel">${escapeHtml2(s.relation)}</span>
      <a href="#" data-id="${escapeHtml2(s.target)}" class="mind-surprise-tgt">${escapeHtml2(nodeLabel(g, s.target))}</a>
  ...
  function renderSearchResultsPanel() {
    ...
    const rows = matchSet.map((id) => {
      ...
      return `<a href="#" data-id="${escapeHtml2(n.id)}" style="display:flex;align-items:baseline;gap:8px;padding:5px 9px;background:var(--base);border-radius:4px;text-decoration:none;color:var(--text);font-size:11px;">
        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;">${escapeHtml2(n.label)}</span>
        <span style="font-size:10px;color:var(--subtext0);">${escapeHtml2(n.kind)}</span>
    ...
    return `
      <div style="background:var(--mantle);border:1px solid var(--surface1);border-radius:6px;padding:10px 12px;margin-bottom:14px;">
        <div style="font-size:10px;font-weight:700;color:var(--subtext0);text-transform:uppercase;letter-spacing:0.6px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:baseline;">
          <span>Search: "${escapeHtml2(state.search)}"</span>
          ...
          <div class="mind-detail-scroll">
            <div class="mind-detail-head">
              <div class="mind-detail-title">
                ...
                <div class="mind-detail-label" title="${escapeHtml2(n.label)}">${escapeHtml2(n.label)}</div>
              </div>
              <button class="tab-bar-btn mind-detail-close" onclick="MindUI.closeDetail()" title="Close">\xD7</button>
            </div>

            <div class="mind-chip-row">
              <span class="mind-chip" style="background:${kindColor2}22;color:${kindColor2};">${escapeHtml2(n.kind)}</span>
              ${n.communityId != null ? `<span class="mind-chip mind-chip-link" data-cid="${escapeHtml2(String(n.communityId))}" style="background:${communityColor(n.communityId)}22;color:${communityColor(n.communityId)};cursor:pointer;" title="Open community #${n.communityId}">community #${n.communityId}</span>` : ""}
              <span class="mind-chip" style="background:${cliC}22;color:${cliC};">${escapeHtml2(cb)}</span>
              ${(n.tags || []).filter((t) => t && t !== n.kind && t !== cb).slice(0, 4).map((t) => `<span class="mind-chip mind-chip-tag">#${escapeHtml2(t)}</span>`).join("")}
            </div>

            <div class="mind-detail-meta">
              ${metaRow("Created", escapeHtml2(created))}
              ${n.source ? renderSource(n.source) : ""}
              ${n.sourceLocation ? renderLocation(n.sourceLocation) : ""}
              ${metaRow("ID", `<code class="mind-id">${escapeHtml2(n.id)}</code>`, true)}
            </div>
            ...
            ${n.preview || n.detail || n.answer || n.result ? `
              <div class="mind-detail-section">
                ...
                <div class="mind-detail-prose">${escapeHtml2((n.preview || n.detail || n.answer || n.result || "").slice(0, 1200))}</div>
              </div>` : ""}
  ...
  async function showCommunityDetail(cid) {
    ...
    detail.innerHTML = `
      <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:8px;">
        <div style="font-size:13px;font-weight:600;color:var(--text);">#${r.id} ${escapeHtml2(r.label || "cluster")}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:3px;">
        ${(r.nodes || []).slice(0, 100).map((n) => `
          <a href="#" class="mind-neighbor-link" data-id="${n.id}" style="display:flex;align-items:baseline;gap:6px;padding:4px 6px;background:var(--base);border-radius:3px;text-decoration:none;font-size:11px;color:var(--text);">
            <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;">${escapeHtml2(n.label)}</span>
            <span style="color:var(--subtext0);font-size:10px;">${escapeHtml2(n.kind)}</span>
          </a>`).join("")}
      </div>`;
    detail.querySelectorAll(".mind-neighbor-link").forEach((a) => {
      a.addEventListener("click", (ev) => {
  ...
  async function askAbout(labelEnc) {
    ...
    detail.innerHTML = `
      <div style="padding:16px;display:flex;flex-direction:column;gap:10px;">
        <div style="font-size:13px;font-weight:600;color:var(--text);">${escapeHtml2(q)}</div>
    ...
    detail.innerHTML = `
      <div class="mind-ask">
        <div class="mind-ask-scroll">
          <div class="mind-ask-q">${escapeHtml2(q)}</div>
          ${answerText ? `<div class="mind-ask-answer">${escapeHtml2(answerText)}</div>
               <div class="mind-ask-model">answered locally by ${escapeHtml2(ans.model || "local model")}${ans.grounded ? " &middot; grounded in " + ans.grounded + " knowledge node" + (ans.grounded === 1 ? "" : "s") : ""}</div>` : `<div class="mind-ask-empty">${escapeHtml2(emptyMsg)}</div>`}
          ${subNodes.length ? `
            <details class="mind-ask-graph" ${answerText ? "" : "open"}>
              ...
              <div class="mind-ask-nodes">
                ${subNodes.slice(0, 50).map((n) => `<a href="#" class="mind-neighbor-link" data-id="${escapeHtml2(n.id)}">${escapeHtml2(n.label)} <span class="mind-ask-kind">(${escapeHtml2(n.kind)})</span></a>`).join("")}
              </div>
            </details>` : ""}
        </div>
        <div class="mind-detail-actions">
