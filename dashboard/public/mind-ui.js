  ...
  (function() {
    ...
    async function copyMermaidSource() {
      ...
      try {
        ...
        const d = await r.json();
        if (!d.mermaid) {
          setStatus(d.error || "no mermaid output");
          return;
        }
      ...
      <div class="mind-card" style="max-width:980px;margin:0 auto;">
        ...
        <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;align-items:center;">
          ...
          <label style="font-size:11px;color:var(--subtext0);display:flex;align-items:center;gap:4px;" title="Approximate token budget for the returned sub-graph">
            budget
            <input id="mindQueryBudget" type="number" min="200" max="8000" step="100" value="${c.budget || 2e3}" style="width:74px;padding:4px 6px;background:var(--surface0);border:1px solid var(--surface1);border-radius:4px;color:var(--text);font-size:11px;">
          </label>
    ...
    function renderCommunities() {
      const g = state.graph;
      let cards = Object.entries(g.communities || {}).map(([cid, c]) => ({ cid, ...c })).sort((a, b) => b.size - a.size);
      if (state.search && state.matches.length) {
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;">
        <div>
          ...
          <div style="display:flex;flex-direction:column;gap:4px;">
            ${gods.slice(0, 25).map((g2) => `
              <a href="#" class="mind-godlink" data-id="${g2.id}" style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;padding:6px 10px;background:var(--mantle);border-radius:4px;text-decoration:none;color:var(--text);font-size:12px;">
                <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(g2.label)}</span>
                <span style="color:var(--subtext0);font-size:10px;font-variant-numeric:tabular-nums;">${g2.degree}</span>
              </a>`).join("")}
          </div>
        </div>
        <div>
          ...
          <div style="display:flex;flex-direction:column;gap:4px;">
            ${surprises.length === 0 ? '<div style="color:var(--subtext0);font-size:11px;font-style:italic;padding:6px;">no cross-community bridges yet - build the brain with more sources</div>' : surprises.slice(0, 25).map((s) => `
              <div style="padding:6px 10px;background:var(--mantle);border-radius:4px;font-size:11px;display:flex;flex-direction:column;gap:2px;">
                ...
                <div style="color:var(--subtext0);font-size:10px;">communities ${s.crossesCommunities.join(" / ")} \xB7 ${s.confidence}</div>
              </div>`).join("")}
          </div>
        </div>
      </div>`;
    ...
    function shadeHex(hex, lum) {
      ...
      if (lum < 0) {
        ...
        r = Math.round(r + (255 - r) * lum);
        g = Math.round(g + (255 - g) * lum);
        b = Math.round(b + (255 - b) * lum);
      }
