// notes -- Notes tab: list/open/edit/preview, slash-AI actions, @-mentions,
// import/export, and the shared customConfirm/customPrompt dialogs + the
// note/file-tree context menus (reveal-in-explorer, rename, duplicate, delete).
//
// esbuild IIFE leaf carved off the flat app.js. Internal helpers stay private;
// the surface reached from index.html handlers, keyboard.js, and other flat
// parts is re-exposed on `window` at the bottom.
//
// IMPORTANT: reads/writes the shared `state` object and registers global
// keydown/click listeners at top level, so it must load AFTER app.js. Its other
// deps resolve as globals: esc/escapeHtml (util), notesFetch/currentNotesNs/
// renderMarkdown/toast/askAi/askAIFromPalette/markOnboarding/_pushFocus (flat
// parts), plus lucide + hljs (vendored).
  ...
  for (const f of files) {
    ...
    if (f.name.endsWith(".json")) {
      try {
        ...
        if (src) {
          for (const [k, v] of Object.entries(src)) if (typeof v === "string") notes[k] = v;
        }
      } catch (_) {
...
window.showNewNoteInput = showNewNoteInput;
window.hideNewNoteInput = hideNewNoteInput;
window.confirmCreateNote = confirmCreateNote;
window.sendNoteToAi = sendNoteToAi;
window.exportCurrentNote = exportCurrentNote;
window.exportAllNotes = exportAllNotes;
window.importNotesFromFile = importNotesFromFile;
window.onNotesImportFileChosen = onNotesImportFileChosen;
...
window.revealCurrentFileInExplorer = revealCurrentFileInExplorer;
window.revealCurrentNoteInExplorer = revealCurrentNoteInExplorer;
window.revealNoteFromContext = revealNoteFromContext;
window.deleteNoteFromContext = deleteNoteFromContext;
window.renameNoteFromContext = renameNoteFromContext;
window.duplicateNoteFromContext = duplicateNoteFromContext;
